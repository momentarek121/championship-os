...
export const tournaments = pgTable("tournaments", {
  ...
  weighInMode: text("weigh_in_mode").$type<"ibjjf" | "custom">().notNull().default("ibjjf"), weighInTolerance: numeric("weigh_in_tolerance", { precision: 6, scale: 2 }).default("0.00").notNull(), competitionMode: text("competition_mode").$type<"gi" | "nogi" | "both">().notNull().default("gi"), scaleNotes: text("scale_notes"), createdBy: bigint("created_by", { mode: "number" }).notNull(),
...
export const matches = pgTable("matches", {
  id: bigint("id", { mode: "number" }).generatedByDefaultAsIdentity().primaryKey(), tournamentId: bigint("tournament_id", { mode: "number" }).notNull(), categoryId: bigint("category_id", { mode: "number" }).notNull(), matId: bigint("mat_id", { mode: "number" }), round: varchar("round", { length: 40 }).notNull(), matchNumber: integer("match_number").notNull(), athleteAId: bigint("athlete_a_id", { mode: "number" }), athleteBId: bigint("athlete_b_id", { mode: "number" }), winnerId: bigint("winner_id", { mode: "number" }), scoreA: integer("score_a").default(0).notNull(), scoreB: integer("score_b").default(0).notNull(), advantageA: integer("advantage_a").default(0).notNull(), advantageB: integer("advantage_b").default(0).notNull(), penaltyA: integer("penalty_a").default(0).notNull(), penaltyB: integer("penalty_b").default(0).notNull(), evaluation: text("evaluation"), status: text("status").$type<"queued" | "called" | "live" | "finished" | "no_show">().notNull().default("queued"), scheduledAt: timestamp("scheduled_at", { withTimezone: true }), durationMinutes: integer("duration_minutes").notNull().default(6), delayMinutes: integer("delay_minutes").notNull().default(0), schedulerOrder: integer("scheduler_order"), finishedAt: timestamp("finished_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(), updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
});
