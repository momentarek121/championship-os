...
- [ ] Replace the raw `Unsupported provider: provider is not enabled` response with a clear bilingual auth message and a safe fallback when Google is disabled.
- [ ] Verify the mobile public homepage and registration flow after the auth-error repair, keeping athlete registration independent from organizer login.
