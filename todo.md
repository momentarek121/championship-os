...
- [ ] Resolve the repeated report that the published Vercel link still does not open or work in the user's browser, with browser-level verification after the fix.
- [ ] Fix the confirmed production Loading-screen loop shown in the user's browser screenshot, then verify that the public entry screen and registration route render beyond Loading.
